...
export default function NettoyageAdminPage() {
  ...
  const [form, setForm] = useState({ _id: null as string | null, subcategory: "homme", name: "", price: "" });
  ...
  const handleSubmit = async () => {
    try {
      ...
      setModalOpen(false);
      setForm({ _id: null, subcategory: "homme", name: "", price: "" }); // Reset form
      fetchData();
    } catch (err: any) {
      console.error(err);
  ...
  return (
    <main className="min-h-screen bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900 text-white p-6">
      ...
      <button
        onClick={() => { setForm({ _id: null, subcategory: "homme", name: "", price: "" }); setModalOpen(true); }}
