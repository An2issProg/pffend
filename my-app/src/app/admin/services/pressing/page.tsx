...
export default function PressingAdminPage() {
  ...
  const [form, setForm] = useState({ _id: null as string | null, subcategory: "costume", name: "", price: "" });
  ...
  const handleSubmit = async () => {
    try {
      ...
      setModalOpen(false);
      setForm({ _id: null, subcategory: "costume", name: "", price: "" }); // Reset form
      fetchData();
    } catch (err: any) {
      console.error(err);
  ...
  return (
    <main className="min-h-screen bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900 text-white p-6">
      ...
      <button
        onClick={() => { setForm({ _id: null, subcategory: "costume", name: "", price: "" }); setModalOpen(true); }}
        ...
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50">
          <div className="bg-gray-800 p-6 rounded-xl w-80">
            ...
            <div className="flex justify-end gap-3">
              ...
              <button onClick={handleSubmit} className="bg-pink-600 px-4 py-1 rounded">{form._id ? "Mettre à jour" : "Enregistrer"}</button>
            </div>
          </div>
        </div>
      )}
